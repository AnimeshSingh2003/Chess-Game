...
if (typeof AFRAME !== 'undefined') {
  AFRAME.registerComponent('reticle-listener', {
    tick: function () {
      ...
      if (this.el.components['ar-hit-test'] && this.el.object3D.visible) {
         document.getElementById('ar-scan-text').innerText = "Surface Detected!";
         document.getElementById('ar-scan-text').style.color = "var(--accent-green)";
         if(btnPlace) btnPlace.disabled = false;
      } else {
         document.getElementById('ar-scan-text').innerText = "Looking for flat surface...";
         document.getElementById('ar-scan-text').style.color = "white";
...
const app = {
  ...
  initChessboards() {
     ...
     this.renderARBoard('ar-board-root', JSON.parse(JSON.stringify(this.initialBoard)));
  },
      ...
      if (this.selectedSquare && this.selectedSquare.containerId === containerId) {
          if (sq !== this.selectedSquare.sq) {
               ...
               if (this.isValidMove(piece, prevR, prevC, r, c, matrix)) {
                   ...
                   if (containerId === 'ar-board-root') {
                       this.renderARBoard(containerId, matrix);
                   } else {
                   ...
                   if(sq.tagName.toLowerCase() === 'a-box') {
                       // Handled implicitly by tracking selectedSquare removing highlight
                   } else {
               ...
               if(sq.tagName.toLowerCase() === 'a-box') {
                   sq.setAttribute('color', sq.getAttribute('data-islight') === 'true' ? '#f0d9b5' : '#b58863');
               } else {
                   sq.classList.remove('highlight');
               }
               this.selectedSquare = null;
          ...
          if (matrix[r][c] !== '') {
              const allSquares = document.querySelectorAll(`#${containerId} .square, #${containerId} a-box`);
              allSquares.forEach(s => {
                  if(s.tagName.toLowerCase() === 'a-box') {
                      s.setAttribute('color', s.getAttribute('data-islight') === 'true' ? '#f0d9b5' : '#b58863');
                  } else {
  ...
  renderARBoard(containerId, matrix) {
     const container = document.getElementById(containerId);
     if (!container) return;
     container.innerHTML = '';
     ...
     for (let r = 0; r < 8; r++) {
       for (let c = 0; c < 8; c++) {
         ...
         sq.setAttribute('width', '0.05');
         sq.setAttribute('height', '0.01');
         sq.setAttribute('depth', '0.05');
         const x = (c * 0.05) - 0.175;
         const z = (r * 0.05) - 0.175;
         sq.setAttribute('position', `${x} 0 ${z}`);
         sq.setAttribute('color', isLight ? '#f0d9b5' : '#b58863');
         sq.setAttribute('data-islight', isLight ? 'true' : 'false');
         sq.classList.add('clickable');
         
         const pieceChar = matrix[r][c];
         if (pieceChar !== '') {
            const img = document.createElement('a-image');
            let srcId = '#' + this.pieceMap[pieceChar].replace('/', '-').replace('.png', '');
            img.setAttribute('src', srcId);
            img.setAttribute('width', '0.04');
            img.setAttribute('height', '0.04');
            img.setAttribute('position', `0 0.026 0`);
            img.setAttribute('rotation', `-45 0 0`);
            img.classList.add('clickable');
            sq.appendChild(img);
            img.addEventListener('click', (e) => {
               e.stopPropagation();
               this.handleSquareClick(sq, r, c, containerId, matrix);
            });
         }
  ...
  // ---- True WebXR AR Mode Logic ----
  
  scanSurface() {
    const scene = document.querySelector('a-scene');
    if (scene && scene.systems.webxr) {
       try {
           scene.systems.webxr.enterXR('ar');
       } catch(e) {
           console.error("XR Error", e);
           this.showToast('WebXR not supported perfectly on this device.');
       }
    }
  },
  ...
  placeARBoard() {
    ...
    const root = document.getElementById('ar-board-root');
    if (reticle && reticle.object3D.visible) {
      root.setAttribute('position', reticle.getAttribute('position'));
      ...
      const btnPlace = document.getElementById('btn-place-board');
      if(btnPlace) btnPlace.disabled = true;
      document.getElementById('ar-scan-text').innerText = "Board Placed";
      document.getElementById('ar-scan-text').style.color = "var(--primary-blue)";
  ...
  resetAR() {
    ...
    const root = document.getElementById('ar-board-root');
    if (reticle) {
      reticle.setAttribute('ar-hit-test', 'target:#ar-board-root; type:footprint');
      reticle.setAttribute('visible', 'false');
    }
    if (root) {
    ...
    const reticleText = document.getElementById('ar-scan-text');
    if(reticleText) {
